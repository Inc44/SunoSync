...
def blend_colors(color_a, color_b, ratio):
	...
	return rgb_to_hex(
		tuple(int(max(0, min(255, a[i] + (b[i] - a[i]) * ratio))) for i in range(3))
	)
	...
	try:
		...
		if metadata_options.get("comment", True) and comment:
			audio.tags["COMM"] = COMM(
				encoding=3, lang="eng", desc="Description", text=comment
			)
		...
		if lyrics_text and metadata_options.get("lyrics", True):
			try:
				...
				print(
					f"Lyrics successfully embedded for {os.path.basename(audio_path)}"
				)
		...
		if image_bytes:
			for key in list(audio.tags.keys()):
				if key.startswith("APIC"):
					del audio.tags[key]
			audio.tags.add(
				APIC(encoding=3, mime=mime, type=3, desc="Cover", data=image_bytes)
			)
	...
	if suppress_sound:
		...
		try:
			...
			if root:
				...
				original_volume = root.tk.call(
					"set", "bell_volume", root.tk.call("set", "bell_volume")
				)
